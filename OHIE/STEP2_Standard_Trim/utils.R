...
basic_trimming<-function(delta,treated_sample,control_sample,flag_discrete,outcome_name,trimming_value,seed,treated_weight,control_weight,
                         treated_responded,control_responded,step) {
  if (delta>=0) {
    print ("delta >0")
...
basic_trimming_itt<-function(data,outcome_name,group_ind=NULL,trimming_value,weight_name=NULL,seed=1,flag_discrete=TRUE,step=10,...) {
  ...
  res<-basic_trimming(delta=delta,treated_sample=treated_sample,
                 control_sample=control_sample,flag_discrete=flag_discrete,outcome_name=outcome_name,
                 trimming_value=trimming_value,seed=seed,treated_weight=treated_weight,control_weight=control_weight,
                 treated_responded=treated_responded,control_responded=control_responded,step=step)
...
basic_trimming_late<-function(data,outcome_name,group_ind=NULL,trimming_value,weight_name=NULL,seed=1,flag_discrete=TRUE,step=10,...) {
  print(outcome_name)
  if (!is.null(group_ind)) {
    data<-data[data$group_id==group_ind,]
    print(dim(data))
  ...
  ## otherwise use full sample
  control_uninsured_sample<-filter(data,treatment==0 & ohp_all_ever_survey ==0 )
  control_insured_sample<-filter(data,treatment==0 & ohp_all_ever_survey ==1 )
  treated_sample<-filter(data,treatment==1)
  
  
  control_uninsured_responded<-!is.na(control_uninsured_sample[,outcome_name])
  control_insured_responded<-!is.na(control_insured_sample[,outcome_name])
  treated_responded<-!is.na(treated_sample[,outcome_name])
  
  
  if (!is.null(weight_name)) {
    control_uninsured_weight<-control_uninsured_sample[,weight_name]
    control_insured_weight<-control_insured_sample[,weight_name]
    treated_weight<-treated_sample[,weight_name]
  } else {
    control_insured_weight<-rep(1,dim(control_sample)[1])
    
    control_uninsured_weight<-rep(1,dim(control_uninsured_sample)[1])
    control_insured_weight<-rep(1,dim(control_insured_sample)[1])
    treated_weight<-rep(1,dim(treated_sample)[1])
    
    weight_name<-"noweight"
    control_uninsured_sample$noweight<-1
    control_insured_sample$noweight<-1
    treated_sample$noweight<-1
  }
  ...
  ## treat-control difference
  delta_uninsured<-weighted.mean(treated_responded,wt=treated_weight)-
    weighted.mean(control_uninsured_responded,wt=control_uninsured_weight) 
  
  res_uninsured<-basic_trimming(delta=delta_uninsured,treated_sample=treated_sample,control_sample=control_uninsured_sample,flag_discrete=flag_discrete,
                                outcome_name= outcome_name,trimming_value=trimming_value,seed=seed,treated_weight=treated_weight,
                                control_weight=control_uninsured_weight,
                           treated_responded= treated_responded,control_responded=control_uninsured_responded,step=step)
  
  delta_insured<-weighted.mean(treated_responded,wt=treated_weight)-
    weighted.mean(control_insured_responded,wt=control_insured_weight) 
  
  res_insured<-basic_trimming(delta=delta_insured,treated_sample=treated_sample,control_sample=control_insured_sample,
                              flag_discrete=flag_discrete,outcome_name=outcome_name,trimming_value=trimming_value,
                              seed=seed,treated_weight=treated_weight,control_weight=control_insured_weight,
                              treated_responded= treated_responded,control_responded=control_insured_responded,step=step)
  
  
  
  ### binary outcome version; low utilization
  
  #control_sample=rbind(res_uninsured$control_sample, res_insured$control_sample)
  #treated_sample=res_uninsured$treated_sample
  #nontrimmed_controls<-c(res_uninsured$nontrimmed_controls,res_insured$nontrimmed_controls)
  
  
  return(list(nontrimmed_controls=c(res_uninsured$nontrimmed_controls,
                                    res_insured$nontrimmed_controls),
              control_sample=filter(data,treatment==0),
              treated_sample=filter(data,treatment==1)
  ))
...
bootstrap_mean_itt_late<-function(b,outcome_name,weight_name,trimming_value,flag_discrete) {
  ...
  for (j in 1:length(unique(prepared_data_sample_12m.boot$group_id)) ) {
    res<-basic_trimming_itt(data=prepared_data_sample_12m.boot,
  ...
  for (j in 1:length(unique(prepared_data_sample_12m.boot$group_id)) ) {
    res<-basic_trimming_late(data=prepared_data_sample_12m.boot,
                            outcome_name= outcome_name, 
                            group_ind=j,
                            trimming_value=trimming_value,
                            step=1,
