...
all_names<-c(utilization_outcomes,numeric_health_outcomes,discrete_outcome_names)
...
for (i in 1:length(all_names)) {
  ...
  for (j in 1:length(unique(prepared_data_sample_12m$group_id)) ) {
    res<-basic_trimming_itt(data=prepared_data_sample_12m,
...
for (i in 1:length(all_names)) {
  ...
  ## column 2
  res<-miceadds::lm.cluster(formula=as.formula(form), data=prepared_data_sample_12m,weights=as.numeric(as.matrix(as.data.frame(prepared_data_sample_12m[,weight_name]))),
                            cluster="household_id")
...
for (i in 1:length(all_names)) {
  ...
  for (j in 1:length(unique(prepared_data_sample_12m$group_id)) ) {
    res<-basic_trimming_late(data=prepared_data_sample_12m,
                            outcome_name= outcome_name, 
                            group_ind=j,
                            trimming_value=trimming_value_all_names[i],
                            step=1,
