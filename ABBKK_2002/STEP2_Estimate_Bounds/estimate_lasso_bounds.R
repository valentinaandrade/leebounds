...
for (subject in c("MATH","READING","WRITING")) {
  ...
  vars_for_outcome<-c(vars_for_outcome,names(fit$coefficients[fit$coefficients!=0]))
  print(names(fit$coefficients[fit$coefficients!=0]))
  print(subject)
...
for (subject in c("MATH","READING","WRITING")) {
  ...
  leebounds_ortho_result<-second_stage_wrapper(leedata=leedata_cov,
                                               ...
                                               inds_helps=first_stage$inds_helps,ortho=TRUE)
  ...
  CR_ortho_postlasso[[subject]]<-c(leebounds_ortho_result$lower_bound_ci,leebounds_ortho_result$upper_bound_ci)
